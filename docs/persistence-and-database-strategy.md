...
### 1단계: 프로토타입은 로컬/임시 저장소

Google AI Studio의 첫 프로토타입은 mock data, localStorage, IndexedDB 같은 임시 저장소로 시작할 수 있다.

목적:

- 화면과 AI 흐름을 빠르게 검증한다.
- 데이터베이스 설정 전에 핵심 UX를 확인한다.
- 프로젝트 대시보드, 추천, 로그 흐름이 실제로 유용한지 본다.

### 2단계: 실제 개인용 MVP는 Firebase Firestore 권장

실제로 개인용으로 계속 사용할 MVP에서는 Firebase Firestore를 권장 앱 DB로 사용한다.

이 문서에서 "앱 DB"라고 부르는 source of truth는 MVP 기준으로 Firebase Firestore를 의미한다.
...
- Google AI Studio/Firebase/Google Workspace 연동 흐름을 같은 생태계 안에서 유지한다.
- 개인용 웹 앱에서 클라우드 저장과 기기 간 접근을 가능하게 한다.

### 3단계: Google Sheets/Calendar는 연동 대상으로 사용
...
### 왜 Firebase Firestore를 권장하는가

Firestore는 이 제품의 MVP에 가장 자연스럽다.

이유:

- Google AI Studio와 Firebase 연동 흐름에 잘 맞는다.
- Firebase Authentication과 함께 개인용 로그인/클라우드 저장을 붙이기 쉽다.
- 문서 기반 데이터 모델이 Project, Task, WorkLog 같은 객체 저장에 적합하다.
- 실시간 동기화와 오프라인 지원을 활용할 수 있다.
- Google Sheets/Calendar 연동과 같은 Google 생태계 연결이 자연스럽다.

주의:

- Firestore Security Rules는 AI가 생성하더라도 반드시 사람이 검토해야 한다.
- 중첩 데이터가 과도해지면 쿼리와 업데이트가 복잡해질 수 있으므로, 주요 객체는 컬렉션으로 분리한다.
- Google Sheets는 Firestore를 대체하지 않고 export/review 대상으로 유지한다.
...
source of truth는 앱 DB다. 실제 MVP에서는 Firebase Firestore를 source of truth로 사용한다.

## 6. Source of Truth 정의

### Firebase Firestore가 원본인 데이터
